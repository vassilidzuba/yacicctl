...
var Cmd = &cobra.Command{
	Use:   "getconfig <project>",
	Short: "get the configuration of a project",
	Long:  `the configuration of a project`,
...
func execute(cmd *cobra.Command, project string) {

	username := viper.GetString("username")
	password := viper.GetString("password")
	host := viper.GetString("host")

	fmt.Println("project getconfig called on", project, "!")

	url := "http://" + host + "/yacic/project/getconfig?project=" + project
	...
	if show {
		...
		err = exec.Command("rundll32", "url.dll,FileProtocolHandler", project + ".json").Run()
		check(err)
	} else {
